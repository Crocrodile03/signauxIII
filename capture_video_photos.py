...
def parse_args():
    p = argparse.ArgumentParser(description="Enregistrer une vidéo et prendre des photos périodiques avec la webcam")
    p.add_argument("--duration", "-d", type=float, default=30.0,
                   help="Durée de la vidéo en secondes (défaut: 30)")
    p.add_argument("--photo-interval", "-i", type=float, default=5.0,
...
def main():
    ...
    if not cap.isOpened():
        print(f"Erreur: impossible d'ouvrir la caméra {args.camera} (backend={args.backend}).")
        return
    ...
    if not writer.isOpened():
        print("Avertissement: VideoWriter n'a pas pu s'ouvrir. Vérifiez le codec ou les permissions.")

    print(f"Enregistrement vidéo: {video_path} durée={args.duration}s fps={fps} taille=({width}x{height})")
